...
export default function WaiterHome() {
  ...
  const renderOrder = (order: Order, tableId: string, index: number) => {
    ...
    let hasItemsToBeDelivered = false;
    ...
    const items: Item[] = extractItemsFromMenu(menu)
      ...
      .map((itm) => {
        ...
        if (recipeOrdered) {
          ...
          itm.invisible = recipeOrdered.status !== "To be delivered";
          itm.index = index;
          if (recipeOrdered.status === "To be delivered")
            hasItemsToBeDelivered = true;
        }
        return itm;
      });

    if (!hasItemsToBeDelivered) return <></>;
